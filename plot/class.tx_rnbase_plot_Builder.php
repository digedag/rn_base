...
class tx_rnbase_plot_Builder
{
    ...
    private function makeCanvas($arrConf)
    {
        ...
        if (function_exists('imageantialias') && 'native' == $arrConf['antialias']) {
     ...
     */
    public function getAltParam($arrConf)
    {
     ...
     */
    public function setMarker(&$objRef, $arrConf)
    {
        if ($arrConf['marker']) {
     ...
     * @param   array       The array with TypoScript properties for the object
     */
    public function setDataPreprocessor(&$objRef, $strType, $arrConf)
    {
        switch ($strType) {
            case 'array':
     ...
     */
    public function setFont(&$objRef, $arrConf)
    {
     ...
     */
    public function setPlotarea(&$objRef, $strValue, $arrConf)
    {
        $Plotarea = '';
        if (is_array($arrConf)) {
     ...
     */
    public function setDataSelector(&$objRef, $strValue, $arrConf)
    {
        switch ($strValue) {
            case 'noZeros':
     ...
     * @param   array       Configuration of the data selector
     */
    public function getAxis(&$objRef, $arrConf)
    {
        $intAxis = 1;
        foreach ($arrConf as $strKey => $strValue) {
            $strKey = rtrim($strKey, '.');
     ...
     * @param   int     Level
     */
    public function setLabelOptions(&$objRef, $arrConf, $intLevel)
    {
        if (is_array($arrConf)) {
            foreach ($arrConf as $strKey => $strValue) {
                if ('showtext' == $strKey || 'showoffset' == $strKey) {
                    $strValue = 1 == $strValue ? true : false;
                }
     ...
     * @param   int     Level
     */
    public function setLabelInterval(&$objRef, $arrConf, $intLevel)
    {
        if (is_array($arrConf)) {
            foreach ($arrConf as $strKey => $strValue) {
     ...
     * @param   array       Configuration of the label
     */
    public function getAxisLevel(&$objRef, $arrConf)
    {
        if (is_array($arrConf)) {
            foreach ($arrConf as $strKey => $strValue) {
                if (intval(rtrim($strKey, '.'))) {
     ...
     */
    public function axisAddMark(&$objRef, $arrConf)
    {
        if (is_array($arrConf)) {
            foreach ($arrConf as $strKey => $strValue) {
                if (intval(rtrim($strKey, '.'))) {
     ...
     */
    public function setPlotareaProperties(&$objRef, $arrConf)
    {
        foreach ($arrConf as $strKey => $strValue) {
            $strKey = rtrim($strKey, '.');
            switch ($strKey) {
     ...
     * @param   array       The array with TypoScript properties for the object
     */
    public function setElementProperties(&$objRef, $arrConf)
    {
        foreach ($arrConf as $strKey => $strValue) {
            $strKey = rtrim($strKey, '.');
            switch ($strKey) {
     ...
     * @param   array       The array with TypoScript properties for the object
     */
    public function setPlotProperties(&$objRef, $arrConf)
    {
        foreach ($arrConf as $strKey => $strValue) {
            $strKey = rtrim($strKey, '.');
            switch ($strKey) {
     ...
     * @param   int     Level
     */
    public function setAxisProperties(&$objRef, $arrConf, $intLevel = 1)
    {
        foreach ($arrConf as $strKey => $strValue) {
            if (false === strpos($strKey, '.') || (false !== strpos($strKey, '.') && !isset($arrConf[rtrim($strKey, '.')]))) {
                $strKey = rtrim($strKey, '.');
                switch ($strKey) {
                    case 'level':
     ...
     */
    public function setAxisMarkerAreaProperties(&$objRef, $arrConf)
    {
        foreach ($arrConf as $strKey => $strValue) {
            $strKey = rtrim($strKey, '.');
            switch ($strKey) {
     ...
     */
    public function setAxisMarkerLineProperties(&$objRef, $arrConf)
    {
        foreach ($arrConf as $strKey => $strValue) {
            $strKey = rtrim($strKey, '.');
            switch ($strKey) {
     ...
     * @param   array       The array with TypoScript properties for the object
     */
    public function setMarkerProperties(&$objRef, $arrConf)
    {
        foreach ($arrConf as $strKey => $strValue) {
            switch ($strKey) {
                case 'size':
     ...
     */
    public function setLegendProperties(&$objRef, $arrConf)
    {
        foreach ($arrConf as $strKey => $strValue) {
            $strKey = rtrim($strKey, '.');
            switch ($strKey) {
