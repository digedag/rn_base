...
class tx_rnbase_util_Misc
{
    ...
    public static function mayday($msg, $extKey = '')
    {
        ...
        $aDebug[] = '<span class="notice"><b>Call 0: </b>' . str_replace(\Sys25\RnBase\Utility\Environment::getCurrentScript(), '/', $aLocation['file']) . ':' . $aLocation['line']  . ' | <b>' . $aTrace1['class'] . $aTrace1['type'] .
                                    $aTrace1['function'] . '</b></span><br/>With parameters: ' . (!empty($aTrace1['args']) ? self::viewMixed($aTrace1['args']) : ' no parameters');
        $aDebug[] = '<hr/>';
        $aDebug[] = '<span class="notice"><b>Call -1: </b>' . str_replace(\Sys25\RnBase\Utility\Environment::getCurrentScript(), '/', $aTrace1['file']) . ':' . $aTrace1['line']  . ' | <b>' . $aTrace2['class'] . $aTrace2['type'] .
                                    $aTrace2['function'] . '</b></span><br />With parameters: ' . (!empty($aTrace2['args']) ? self::viewMixed($aTrace2['args']) : ' no parameters');
        $aDebug[] = '<hr/>';
        $aDebug[] = '<span class="notice"><b>Call -2: </b>' . str_replace(\Sys25\RnBase\Utility\Environment::getCurrentScript(), '/', $aTrace2['file']) . ':' . $aTrace2['line']  . ' | <b>' . $aTrace3['class'] . $aTrace3['type'] .
                                    $aTrace3['function'] . '</b></span><br />With parameters: ' . (!empty($aTrace3['args']) ? self::viewMixed($aTrace3['args']) : ' no parameters');
        $aDebug[] = '<hr/>';
        $aDebug[] = '<span class="notice"><b>Call -3: </b>' . str_replace(\Sys25\RnBase\Utility\Environment::getCurrentScript(), '/', $aTrace3['file']) . ':' . $aTrace3['line']  . ' | <b>' . $aTrace4['class'] .
    ...
    public static function prepareTSFE($options = array())
    {
        ...
        if (!is_object($GLOBALS['TSFE']) ||
            ...
            if (!defined('PATH_tslib')) {
                // PATH_tslib setzen
                if (@is_dir(\Sys25\RnBase\Utility\Environment::getCurrentScript().'typo3/sysext/cms/tslib/')) {
                    define('PATH_tslib', \Sys25\RnBase\Utility\Environment::getCurrentScript().'typo3/sysext/cms/tslib/');
                } elseif (@is_dir(\Sys25\RnBase\Utility\Environment::getCurrentScript().'tslib/')) {
                    define('PATH_tslib', \Sys25\RnBase\Utility\Environment::getCurrentScript().'tslib/');
                } else {
                    define('PATH_tslib', '');
                }
            }
        ...
        if (!is_object($GLOBALS['TSFE']->tmpl) || $force) {
            ...
            if (empty($GLOBALS['TSFE']->tmpl->getFileName_backPath)) {
                $GLOBALS['TSFE']->tmpl->getFileName_backPath = \Sys25\RnBase\Utility\Environment::getCurrentScript();
            }
        }
