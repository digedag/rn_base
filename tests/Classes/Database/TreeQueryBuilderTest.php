...
class Tx_Rnbase_Database_TreeQueryBuilderTest extends Tx_Phpunit_TestCase
{
	...
	public function testGetTreeRecursive()
	{
		...
		$treeQueryBuildMock = $this->getTreeQueryBuilderMock(function ($connection) {
			$connection->expects(self::at(0))
				->method('doSelect')
				->with('uid', 'pages', array('where' => 'pid IN (1)', 'tableName' => 'pages'))
				->willReturn(array(array('uid' => 2)));

			$connection->expects(self::at(1))
				->method('doSelect')
				->with('uid', 'pages', array('where' => 'pid IN (2)', 'tableName' => 'pages'))
				->willReturn(array(array('uid' => 3), array('uid' => 6)));

			$connection->expects(self::at(2))
				->method('doSelect')
				->with('uid', 'pages', array('where' => 'pid IN (3)', 'tableName' => 'pages'))
				->willReturn(array(array('uid' => 4)));

			$connection->expects(self::at(3))
				->method('doSelect')
				->with('uid', 'pages', array('where' => 'pid IN (4)', 'tableName' => 'pages'))
				->willReturn(array());

			$connection->expects(self::at(4))
				->method('doSelect')
				->with('uid', 'pages', array('where' => 'pid IN (6)', 'tableName' => 'pages'))
				->willReturn(array(array('uid' => 7)));

			$connection->expects(self::at(5))
				->method('doSelect')
				->with('uid', 'pages', array('where' => 'pid IN (7)', 'tableName' => 'pages'))
				->willReturn(array());
		});


		$uidList = $treeQueryBuildMock->getTreeListRecursive(1, 4);
	...
	public function testLimitedTreeByDepth()
	{
		...
		$uidList = $treeQueryBuildMock->getTreeListRecursive(1, 2);
	...
	public function testAddPidToCustomQueryCorrectly()
	{
		...
		$treeQueryBuildMock = $this->getTreeQueryBuilderMock(function ($connection) {
			$connection->expects(self::at(0))
				->method('doSelect')
				->with('uid', 'pages', array('where' => 'hidden=1 AND pid IN (1)', 'tableName' => 'pages'))
				->willReturn(array(array('uid' => 2)));

			$connection->expects(self::at(1))
				->method('doSelect')
				->with('uid', 'pages', array('where' => 'hidden=1 AND pid IN (2)', 'tableName' => 'pages'))
	...
	public function testSetCustomTableNameCorrectly()
	{
		...
		$treeQueryBuildMock = $this->getTreeQueryBuilderMock(function ($connection) {
			$connection->expects(self::at(0))
				->method('doSelect')
				->with('uid', 'tt_content', array('where' => 'pid IN (1)', 'tableName' => 'tt_content'))
				->willReturn(array(array('uid' => 33), array('uid' => 44)));

			$connection->expects(self::at(1))
				->method('doSelect')
				->with('uid', 'tt_content', array('where' => 'pid IN (33)', 'tableName' => 'tt_content'))
				->willReturn(array());

			$connection->expects(self::at(2))
				->method('doSelect')
				->with('uid', 'tt_content', array('where' => 'pid IN (44)', 'tableName' => 'tt_content'))
				->willReturn(array());
		});
	...
	{
		$options = array(
			'where' => '(starttime > 1234567890 AND endtime < 9876543210)',
			'tableName' => 'tt_content',
			'orderby' => 'header',
			'limit' => 1,
		);
		...
		$treeQueryBuildMock = $this->getTreeQueryBuilderMock(function ($connection) {
			$connection->expects(self::at(0))
				...
				->with(
					...
					'tt_content',
					array(
						'where' => '(starttime > 1234567890 AND endtime < 9876543210) AND pid IN (1)',
						'tableName' => 'tt_content',
						'orderby' => 'header',
						'limit' => 1
					)
			...
			$connection->expects(self::at(1))
				...
				->with(
					...
					'tt_content',
					array(
						'where' => '(starttime > 1234567890 AND endtime < 9876543210) AND pid IN (2)',
						'tableName' => 'tt_content',
						'orderby' => 'header',
						'limit' => 1
					))
	...
	public function testSetCustomParentField()
	{
		...
		$treeQueryBuildMock = $this->getTreeQueryBuilderMock(function ($connection) {
			$connection->expects(self::at(0))
				->method('doSelect')
				->with('uid', 'pages', array('where' => 'parent_id IN (1)', 'tableName' => 'pages', 'parentField' => 'parent_id'))
				->willReturn(array(array('uid' => 5)));

			$connection->expects(self::at(1))
				->method('doSelect')
				->with('uid', 'pages', array('where' => 'parent_id IN (5)', 'tableName' => 'pages', 'parentField' => 'parent_id'))
				->willReturn(array());
	...
	public function testSetCustomKeyField()
	{
		...
		$treeQueryBuildMock = $this->getTreeQueryBuilderMock(function ($connection) {
			$connection->expects(self::at(0))
				->method('doSelect')
				->with('entity_id', 'pages', array('where' => 'pid IN (1)', 'tableName' => 'pages', 'idField' => 'entity_id'))
				->willReturn(array(array('entity_id' => 5)));

			$connection->expects(self::at(1))
				->method('doSelect')
				->with('entity_id', 'pages', array('where' => 'pid IN (5)', 'tableName' => 'pages', 'idField' => 'entity_id'))
				->willReturn(array());
